...
def main():
    ...
    targets = df_comb.target.values.astype(str)  # df.target[~df.target.isnull()].unique()
    utargets = np.unique(targets)
    utarget_len = len(utargets)
    ...
    compound_target_remap = {}
    for i, t in tqdm(enumerate(utargets), total=utarget_len, desc="Targets"):
        ikeys = df_comb[targets == t].inchi_key.values
    ...
    target_id_remap = {c: idx for idx, c in enumerate(uni_cats)}
    train_y = np.asarray([target_id_remap[x] for x in train_y])
    test_y = np.asarray([target_id_remap[x] for x in test_y])

    # Save metadata
    np.savez(
        os.path.join(eval_data_dir, "target_data.npz"),
        train_X=train_X,
        train_y=train_y,
        test_X=test_X,
        test_y=test_y,
        id_remap=target_id_remap,
        compound_remap=compound_target_remap,
