...
if model_type == "torch":
    ...
    # with torch.no_grad():
    #     proj_X = []
    #     for it_X in tqdm(data, total=len(data), desc="Projecting full data"):
    #         it_X = torch.from_numpy(it_X[None]).cuda()
    #         z = model(it_X)
    #         proj_X.append(z.cpu())
    #     proj_X = np.concatenate(proj_X, 0)

    with torch.no_grad():
        ptrain_X, check_y, check_names = [], [], []
        for it_X, it_y, it_n in tqdm(zip(train_X, train_y, y_names), total=len(train_X), desc="Projecting train data"):
            it_X = torch.from_numpy(it_X[None]).cuda()
            z = model(it_X)
            ptrain_X.append(z.cpu())
            check_y.append(it_y)
            check_names.append(it_n)
        ptrain_X = np.concatenate(ptrain_X, 0)
        check_y = np.asarray(check_y)
        check_names = np.asarray(check_names)

    # Gen libs
    # inv_remap = {v: k for k, v in remap.items()}
    sel_cat = remap["antioxidant"]
    subset_rad = np.where(train_y == sel_cat)[0]
    sel_keys = train_key[subset_rad]
    check_moa = y_names[subset_rad]
    query = ptrain_X[subset_rad]
    ...
    omni_smile_key_df.to_csv("omnibus_antioxidant_hits_v{}.csv".format(version))

    # # Build libs per compound
    # uni_keys = np.unique(sel_keys)[:5]
    # for k in tqdm(uni_keys, total=len(uni_keys), desc="Processing hits"):
    #     subset_rad = np.where(train_key == k)[0]
    #     query = ptrain_X[subset_rad]
    #     smile_key_df = ranked_library(
    #         query=query,
    #         db=ptrain_X,
    #         keys=train_key,
    #         smiles=train_smile,
    #         metric="cosine")
    #     clean_idx = np.in1d(smile_key_df.inchi_keys.values, sel_keys)
    #     smile_key_df = smile_key_df[~clean_idx]  # Remove keys in our query
    #     smile_key_df.to_csv("{}_antioxidant_hits_v{}.csv".format(version)))

    # Build a control lib
    cancer_control_key = "HHZIURLSWUIHRB-UHFFFAOYSA-N"
    cancer_control_query = ptrain_X[np.where(train_key == cancer_control_key)[0]]
    cancer_control_smile_key_df = ranked_library(
        query=cancer_control_query,
        db=ptrain_X,
        keys=train_key,
        smiles=train_smile,
        metric="cosine")
    clean_idx = np.in1d(cancer_control_smile_key_df.inchi_keys.values, cancer_control_key)
    cancer_control_smile_key_df = cancer_control_smile_key_df[~clean_idx]  # Remove keys in our query
    cancer_control_smile_key_df.to_csv("tyrosine_kinase_hits_v{}.csv".format(version))

# import requests
# from rdkit import Chem
# from rdkit.Chem import AllChem

# # Define the SELFIES string
# smiles_string = "CC(C)C1C(=O)N(S(C)(=O)=O)C2CCN(C(=O)C=CCN3CCCCC3)C12"

# # Make API call to PubChem to get protein targets
# url = f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/smiles/{smiles_string}/JSON?record_type=3d"
# response = requests.get(url)
# response_data = response.json()

# # Extract protein targets from the response data
# protein_targets = []
# if "ProteinGI" in response_data:
#     protein_targets = response_data["ProteinGI"]

# # Print protein targets
# for target in protein_targets:
#     print(target)

# files = glob("*moa*{}.csv".format(version)) 
