...
class Mmd_resnet(nn.Module):   
    def __init__(self, 
        ...
        # Here add the Eval head
        self.final = nn.Sequential(*[
            torch.nn.Linear(int_dim, hidden),
    ...
    def forward(self, dv, return_p = False):
...
def main(
    ...
    moa_perf = eval_tools.run(kind="moa", train_X=train_enc, train_y=train_lab, test_X=test_enc, test_y=test_lab, device=device, epochs=test_epochs, width=width, sel_train=sel_train, sel_test=sel_test, model=model)
    ...
    results = {
        ...
        "z_prime_orf": 0,  # z_prime_orf,
        "z_prime_crispr": 0,  # z_prime_crispr,
        "d_orf": 0,  # d_orf,
        "d_crispr": 0,  # d_crispr
    }
    db_utils.record_performance(results, cp=True)
...
if __name__ == '__main__':
    ...
    if args.debug:
        ...
        # Run in DB mode
        params = db_utils.get_and_reserve_params(cp=True)
